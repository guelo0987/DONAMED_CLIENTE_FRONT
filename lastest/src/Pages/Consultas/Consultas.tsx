import { ChevronDown, Search } from "lucide-react";
import { MainLayout } from "../../components/layout/MainLayout";
import { Button } from "../../components/ui/buttons";
import { MedicationDetailModal } from "../../components/ui/medication-detail-modal";
import { useState } from "react";

interface MedicationItem {
    name: string;
    category: string;
    image: string;
    subtitle: string;
    treatment: string;
    itemCode: string;
    description: string;
}

const medications: MedicationItem[] = [
    {
        name: "Tecentriq",
        category: "Oncológico",
        image: "/medicines/tecentriq.png",
        subtitle: "Avastin 150mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908763",
        description:
            "Es un anticuerpo monoclonal que se dirige a las células que sobreexpresan la proteína HER2. Utilizado en el tratamiento de ciertos tipos de cáncer de mama y cáncer gástrico.",
    },
    {
        name: "Herceptin",
        category: "Oncológico",
        image: "/medicines/herceptin.png",
        subtitle: "Trastuzumab 150mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908764",
        description:
            "Medicamento oncológico utilizado en terapias dirigidas para pacientes con cáncer de mama HER2 positivo.",
    },
    {
        name: "Floxid Cure",
        category: "Oncológico",
        image: "/medicines/floxid_cure.png",
        subtitle: "FloxidCare 500mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908765",
        description:
            "Antibiótico de amplio espectro para infecciones complejas y tratamientos prolongados.",
    },
    {
        name: "Perjeta",
        category: "VIH/SIDA",
        image: "/medicines/perjeta.png",
        subtitle: "Pertuzumab 420mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908766",
        description:
            "Terapia dirigida utilizada en combinación para cáncer de mama HER2 positivo.",
    },
    {
        name: "Herceptin Trastuzumab",
        category: "VIH/SIDA",
        image: "/medicines/herceptin_trastuzumab.png",
        subtitle: "Trastuzumab 440mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908767",
        description:
            "Tratamiento oncológico que ayuda a bloquear el crecimiento de células con HER2.",
    },
    {
        name: "Leucovorina Calcica Tuteur",
        category: "Oncológico",
        image: "/medicines/leucovorina_calcica_tuteur.png",
        subtitle: "Leucovorina 20mg",
        treatment: "Tratamiento Intravenoso",
        itemCode: "4030008908768",
        description:
            "Medicamento coadyuvante utilizado para reducir efectos secundarios de quimioterapia.",
    },
];

export const Consultas = () => {
    const [selectedMedication, setSelectedMedication] =
        useState<MedicationItem | null>(null);

    return (
        <MainLayout className="bg-[#FFFFFF]">
            <section className="w-full px-4 py-6 lg:py-10">
                <div className="max-w-[1200px] mx-auto">
                    <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-6 lg:gap-10 items-center lg:justify-items-center">
                        <div className="space-y-10 max-w-[700px]">
                            <h1 className="text-[#2D3748] text-[40px] md:text-[40px] xl:text-[44px] font-semibold leading-tight">
                                <span className="text-[#34A4B3] font-['Merienda']">
                                    Encuentra{" "}
                                </span>
                                el apoyo que necesitas para tu bienestar
                            </h1>

                            <div className="bg-white backdrop-blur-[20px] rounded-[20px] lg:rounded-[22px] shadow-[0px_18px_40px_rgba(0,0,0,0.08)] p-5 lg:p-6 border border-gray-50/50 max-w-[700px]">
                                <p className="text-[#404040] font-semibold text-[12px] lg:text-[14px] mb-4 [font-family:'Poppins',sans-serif] ml-2 text-center lg:text-left">
                                    Encuentra medicamentos disponibles
                                </p>
                                <div className="flex flex-col lg:flex-row gap-4 items-center">
                                    <div className="flex-[2] w-full">
                                        <input
                                            type="text"
                                            placeholder="Nombre del Medicamento"
                                            className="w-full bg-[#F3F4F6] rounded-full px-6 lg:px-7 py-4 lg:py-4.5 text-[#4A5568] placeholder:text-[#A0AEC0] text-[13px] lg:text-[14px] [font-family:'Poppins',sans-serif] outline-none focus:ring-2 focus:ring-[#40C9DB]/30 transition-all"
                                        />
                                    </div>
                                    <div className="flex-[1] w-full relative">
                                        <select className="w-full bg-[#F3F4F6] rounded-full px-6 lg:px-7 py-3 lg:py-3.5 text-[#4A5568] text-[13px] lg:text-[14px] [font-family:'Poppins',sans-serif] outline-none focus:ring-2 focus:ring-[#40C9DB]/30 appearance-none cursor-pointer pr-12 transition-all">
                                            <option>Filtros</option>
                                            <option>Disponible</option>
                                            <option>No disponible</option>
                                        </select>
                                        <ChevronDown className="absolute right-6 top-1/2 -translate-y-1/2 w-4 h-4 lg:w-5 lg:h-5 text-[#718096] pointer-events-none" />
                                    </div>
                                    <button className="bg-[#34A4B3] text-white px-7 lg:px-9 py-3 lg:py-3.5 rounded-full flex items-center justify-center gap-2 [font-family:'Poppins',sans-serif] font-semibold text-[13px] lg:text-[14px] hover:bg-[#2B93A1] transition-all transform hover:scale-[1.02] shadow-lg shadow-[#34A4B3]/30 w-full lg:w-auto min-w-[140px]">
                                        <Search className="w-4 h-4 lg:w-5 lg:h-5" />
                                        Buscar
                                    </button>
                                </div>
                            </div>
                        </div>

                        <div className="relative hidden lg:flex items-center justify-center w-full">
                            <img
                                src="/banners/Banner%20de%20Consulta.png"
                                alt="Banner de consulta"
                                className="w-full max-w-[420px] h-auto object-contain mx-auto"
                            />
                        </div>
                    </div>
                </div>
            </section>

            <section className="w-full px-4 pb-10 mt-10">
                <div className="max-w-[1200px] mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 gap-y-8">
                    {medications.map((medication, index) => (
                        <div
                            key={`${medication.name}-${index}`}
                            className="bg-white rounded-[20px] shadow-[0px_10px_28px_rgba(0,0,0,0.12)] border border-[#EFEFEF] px-6 py-8 flex flex-col items-center text-center h-[410px] w-full max-w-[300px] mx-auto transition-all duration-300 hover:-translate-y-1 hover:shadow-[0px_16px_36px_rgba(0,0,0,0.14)]"
                        >
                            <div className="relative w-full h-[215px] flex items-center justify-center">
                                <img
                                    src="/assets/Rectangulo%20Medicamentos.png"
                                    alt=""
                                    className="absolute w-[230px] h-auto object-contain translate-y-12"
                                />
                                <img
                                    src={medication.image}
                                    alt={medication.name}
                                    className="relative max-h-[220px] object-contain -translate-y-2"
                                />
                            </div>
                            <div className="mt-3">
                                <p className="text-[#404040] text-[14px] xl:text-[15px] font-medium">
                                    {medication.name}
                                </p>
                                <p className="text-[#404040] text-[12px] xl:text-[13px] opacity-80">
                                    {medication.category}
                                </p>
                            </div>
                            <Button
                                onClick={() => setSelectedMedication(medication)}
                                className="mt-6 w-[130px] h-[36px] bg-[#34A4B3] hover:bg-[#2d8f9c] rounded-[12px] text-white text-[12px] xl:text-[13px] font-medium shadow-none hover:shadow-lg transition-all"
                            >
                                Ver detalles
                            </Button>
                        </div>
                    ))}
                </div>
            </section>

            <MedicationDetailModal
                open={!!selectedMedication}
                onClose={() => setSelectedMedication(null)}
                name={selectedMedication?.name ?? ""}
                image={selectedMedication?.image ?? ""}
                subtitle={selectedMedication?.subtitle}
                treatment={selectedMedication?.treatment}
                itemCode={selectedMedication?.itemCode}
                category={selectedMedication?.category}
                description={selectedMedication?.description}
            />
        </MainLayout>
    );
};
